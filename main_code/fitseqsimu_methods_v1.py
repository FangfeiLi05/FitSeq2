    ...
    def function_growth_lineage(self, data_lineage_dict): #growth
        ...
        if (not isinstance(data_lineage_dict, dict) or not data_lineage_dict): #final layer
            if data_lineage_dict[0] > 0:
                #data_lineage_dict[0] = 2 * data_lineage_dict[0] * (1 + data_lineage_dict[1]) # Writgtian fitness
                data_lineage_dict[0] = 2 * data_lineage_dict[0] * np.exp(data_lineage_dict[1]) # Mathusian fitness
    ...
    def function_main(self):
        ...
        for t in tqdm(range(self.evo_num + 1)):
            ...
            if t == 0: 
                ...
                self.data_dict = {i: [self.n0_array[i], self.s_array[i]] for i in range(self.lineages_num)}
                # [a0, a1]: -- a0: number of individuals
                #           -- a1: fitness of each individual (sum of fitness of all mutations)
                
                self.data_saturated_dict = {i: [self.n0_array[i]*2**self.delta_t, self.s_array[i]] for i in range(self.lineages_num)}
